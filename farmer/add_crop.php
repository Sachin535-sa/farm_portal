...
if(isset($_POST['save_crop'])){
    ...
    $auction_duration = mysqli_real_escape_string($conn, $_POST['auction_duration']);
    ...
    if (!empty($expiry_date) && strtotime($expiry_date) < strtotime(date("Y-m-d"))) {
        $error_msg = "⚠️ Invalid Expiry Date! The crop cannot be already expired.";
    }
    ...
    if (isset($_FILES['crop_image']) && $_FILES['crop_image']['error'] == 0) {
        ...
        if (in_array($ext, $allowed)) {
            ...
            $error_msg = "⚠️ Invalid image format! Please upload JPG, PNG, or WEBP.";
        }
    }
    ...
    if (empty($error_msg)) {
        if($is_edit){
            ...
            $sql = "UPDATE crops SET crop_name='$crop_name', price='$price', quantity='$quantity', 
                    ...
                    sustainability_badges='$sustainability_badges', is_flagged='$is_flagged', flag_reason='$flag_reason', is_auction='$is_auction', starting_bid='$starting_bid', auction_end_time='$auction_end_time'
                    $img_part 
            ...
            if(mysqli_query($conn, $sql)){
                ...
                $error_msg = "⚠️ Error updating crop: " . mysqli_error($conn);
            }
        } else {
            $sql = "INSERT INTO crops(farmer_id, crop_name, price, quantity, crop_image, expiry_date, ref_reliance_price, ref_bigbasket_price, ref_mandi_price, sustainability_badges, is_flagged, flag_reason, is_auction, starting_bid, auction_end_time)
                    VALUES('$farmer_id', '$crop_name', '$price', '$quantity', '$crop_image', '$expiry_date', '$ref_reliance_price', '$ref_bigbasket_price', '$ref_mandi_price', '$sustainability_badges', '$is_flagged', '$flag_reason', '$is_auction', '$starting_bid', '$auction_end_time')";
            if(mysqli_query($conn, $sql)){
                $msg = "🌾 Crop listed successfully!";
                if ($is_flagged == 1) {
                    $msg .= " (⚠️ Note: This crop has been flagged by the automated safety system for suspicious pricing reviews.)";
                }
                ...
                $error_msg = "⚠️ Error adding crop: " . mysqli_error($conn);
            }
        }
    }
}
    ...
    <div class="auth-card animate-slide" style="max-width: 500px; text-align: left;">
        ...
        <div style="text-align: center; margin-bottom: 24px;">
            ...
            <h2 style="font-size: 24px; color: var(--dark); margin-top: 8px;">
                <?php echo $is_edit ? '⚙️ Edit Crop Listing' : '➕ List Your Crop'; ?>
            </h2>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="form-label" for="expiry_date">Product Expiry Date 📅</label>
            ...
            <div style="background: rgba(15, 118, 110, 0.04); border: 1px dashed rgba(15, 118, 110, 0.15); padding: 16px; border-radius: var(--radius-sm); margin-bottom: 24px;">
                <h4 style="font-size: 13.5px; color: var(--secondary); margin: 0 0 10px 0; text-transform: uppercase; letter-spacing: 0.5px; display: flex; align-items: center; gap: 6px;">
                    🛒 Supermarket Price Benchmarks (Optional)
                </h4>
                <p style="font-size: 11.5px; color: var(--text-muted); margin: 0 0 14px 0; line-height: 1.4;">
                ...
                <div class="form-group" style="margin-bottom: 0;">
                    ...
                    <!-- Auction Settings -->
                    <div class="auction-section <?php echo $is_auction ? '' : 'hidden'; ?>">
                        <div class="form-group" style="margin-bottom: 12px;">
                            <label class="form-label" for="is_auction" style="font-size: 12px;">
                                <input type="checkbox" id="is_auction" name="is_auction" <?php echo $is_auction ? 'checked' : ''; ?>> Enable Auction
                            </label>
                        </div>
                        <div class="form-group" style="margin-bottom: 12px;">
                            <label class="form-label" for="starting_bid" style="font-size: 12px;">Starting Bid (₹)</label>
                            <input class="form-control" type="number" min="1" id="starting_bid" name="starting_bid" value="<?php echo htmlspecialchars($starting_bid); ?>" placeholder="e.g., 50">
                        </div>
                        <div class="form-group" style="margin-bottom: 12px;">
            ...
            <div style="display: flex; gap: 14px;">
                ...
                <button type="submit" name="save_crop" class="btn btn-primary" style="flex: 1.5; justify-content: center;">
                    <?php echo $is_edit ? 'Save Changes' : 'Publish Listing 🚀'; ?>
                </button>
            </div>
